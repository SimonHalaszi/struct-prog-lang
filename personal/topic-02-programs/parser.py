...
def test_parse_factor():
    ...
    # Added test cases
    for s in ["2", "450", "123054"]:
        tokens = tokenize(s)
        ast, tokens = parse_factor(tokens)
        assert ast == {'tag': 'number', 'value': int(s)}
        assert tokens[0]['tag'] == None 

    for s in ["(1032)","(5402)", "(54302)"]:
        tokens = tokenize(s)
        ast, tokens = parse_factor(tokens)
        s_n = s.replace("(","").replace(")","")
        assert ast=={'tag': 'number', 'value': int(s_n)}
    ...
    tokens = tokenize("(4*10)")
    ast, tokens = parse_factor(tokens)
    assert ast == {'tag': '*', 'left': {'tag': 'number', 'value': 4}, 'right': {'tag': 'number', 'value': 10}}
...
def test_parse_term():
    ...
    # Added test cases
    for s in ["543","24532","6542"]:
        tokens = tokenize(s)
        ast, tokens = parse_term(tokens)
        assert ast=={'tag': 'number', 'value': int(s)}
        assert tokens[0]['tag'] == None 
    
    tokens = tokenize("328/2")
    ast, tokens = parse_term(tokens)
    assert ast == {'tag': '/', 'left': {'tag': 'number', 'value': 328}, 'right': {'tag': 'number', 'value': 2}}
    
    tokens = tokenize("1/320*78")
    ast, tokens = parse_term(tokens)
    assert ast == {'tag': '*', 'left': {'tag': '/', 'left': {'tag': 'number', 'value': 1}, 'right': {'tag': 'number', 'value': 320}}, 'right': {'tag': 'number', 'value': 78}}
